...
async fn main(spawner: Spawner) {
    ...
    let mut config = UsbConfig::new(0xc0de, 0x0004);
    config.manufacturer = Some("dick mouse");
    config.product = Some("UAC1 USB-only speaker sample");
    config.serial_number = Some("speaker-usb-only-0001");
    ...
    println!("speaker: USB-only UAC1 48k S16_LE ready (UART0 115200 8N1)");
