    ...
    if ints.iepint() {
        ...
        while ep_mask != 0 {
            if ep_mask & 1 != 0 {
                ...
                // Count hardware XFRC here, before its W1C acknowledgement. TXFE,
                // EPDISD and incomplete-ISO retries are NOT completed transfers.
                if ep_ints.xfrc() {
                    state.ep_states[ep_num].in_completed.fetch_add(1, Ordering::Relaxed);
                }
    ...
    M: RawMutex + Copy,
{
    ...
    /// Non-control IN packets queued and hardware XFRC interrupts acknowledged.
    /// Counters wrap at u32::MAX and survive USB resets, but not MCU resets.
    /// XFRC is not proof of a nonempty PCM packet arriving at the host.
    pub fn in_transfer_counts(&self, index: usize) -> Option<(u32, u32)> {
