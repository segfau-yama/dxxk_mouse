...
/// Implementation of a fixed-48 kHz asynchronous Audio Source interface.
///
/// The stream has one isochronous IN data endpoint and no feedback endpoint.
...
impl<'d, D: Driver<'d>> AudioSource<'d, D> {
    ...
    fn create_streaming_iface_active(
        ...
        // This source is intentionally fixed-rate. An async IN endpoint has no
        // synchronization/feedback endpoint, so advertise exactly one rate.
        assert!(
            sample_rates.len() == 1 && sample_rates[0] == FIXED_SAMPLE_RATE_HZ,
            "AudioSource requires a fixed 48 kHz sample rate"
        );
        let sample_rate = FIXED_SAMPLE_RATE_HZ;

        // Format Type I Descriptor (UAC1, 4.5.1) for one sample rate.
        // Total Length: 11 bytes (8 + 3). Body: 9 bytes.
        let format_type_i_body: [u8; 9] = [
            ...
            (sample_rate & 0xFF) as u8,      // tSamFreq[0]: LSB
        ...
        let max_packet_size: u16 =
            calculate_max_packet_size(sample_rate, MAX_AUDIO_CHANNEL_COUNT as u8, sample_width as u8);
        ...
        b.endpoint_descriptor(
            ...
            &[0x00, 0x00], // bRefresh=0 and bSynchAddress=0: no feedback endpoint.
        );
        ...
        b.descriptor(
            ...
            &[
                ...
                0x00,       // bLockDelayUnits: undefined for asynchronous source
                0x00,       // wLockDelay[0]
                0x00,       // wLockDelay[1]
            ],
        );
    ...
    pub fn new(
        ...
        let ep_audio_in = Self::create_streaming_iface_active(&mut alt_setting, sample_rates, sample_width);
...
impl AudioSourceControlHandler {
    ...
    fn handle_ep_in<'a>(&mut self, req: Request, buf: &'a mut [u8]) -> Option<InResponse<'a>> {
        ...
        match req.request {
            GET_CUR => {
                // Optional sampling-frequency control on the audio endpoint.
